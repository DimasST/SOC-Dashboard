...
import { LogItem, SensorData } from "./lib/types";
...
export default function Page() {
  ...
  const groupByDevice = allLogs.reduce<Record<string, LogItem[]>>((acc, log) => {
    ...
    <div className="min-h-screen bg-[#0D1B2A] text-white p-6 font-sans">
      ...
      <div className="bg-[#1B263B] p-4 rounded mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <SensorChart title="Total Device" data={Object.entries(groupByDevice).map(([device, sensors]) => ({ name: device, value: sensors.length, unit: "" }))} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#1B263B] p-4 rounded">
          ...
          {[...logs.slice(0, 5), ...websiteLogs.slice(0, 5)].map((sensor, index) => (
            <div
