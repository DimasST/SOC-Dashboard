          ...
          {downLogs.map((log, i) => {
            ...
            return (
              <li key={`${log.objid}-alert-${i}`}>
                [{time}] ❌ Sensor <strong>{log.sensor}</strong> di device <strong>{log.device}</strong> status: <strong>{log.status}</strong>, nilai: <strong>{log.lastvalue}</strong>
              </li>
            );
          })}
        </ul>
          ...
          {recentLogs.map((log, i) => {
            ...
            return (
              <li key={`${log.objid}-log-${i}`}>
                [{time}] {icon} Sensor <strong>{log.sensor}</strong> di device <strong>{log.device}</strong> status: <strong>{log.status}</strong>, nilai: <strong>{log.lastvalue}</strong>
              </li>
            );
          })}
        </ul>
